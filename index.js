...
function updateChart(speed, days) {
    ...
    if (!ctx) {
        console.error('Canvas element not found! Please check if <canvas id="downloadChart"> exists.');
        return;
    }

    if (window.downloadChart) {
        window.downloadChart.destroy();
    }

    window.downloadChart = new Chart(ctx.getContext('2d'), {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
...
document.getElementById('downloadForm').addEventListener('submit', function (event) {
    ...
    console.log('Updating chart with speed:', speed, 'days:', days);
